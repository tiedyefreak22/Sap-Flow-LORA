...
for trial = 1:1:trials
    ...
    for window = 1:1:length(windows)
        ...
        for i = 1:length(decoded_messages)
            ...
            BER(window, trial, i) = numErrors / length(inputArray);
        ...
        if trial == 1
            cum_BER(window, trial) = sum(BER(window, trial, :));
        else
            cum_BER(window, trial) = cum_BER(window, trial - 1) + sum(BER(window, trial, :));
        end
...
figure
hold on
title("BER vs. Window by Trial")
for i = 1:1:size(BER, 1)
    x = 1:1:trials;
    plot(x, sum(BER(i, :, :), 3))
    legend(windows)
end
...
figure
hold on
title("Cumulative BER vs. Window")
for i = 1:1:size(cum_BER, 1)
    x = 1:1:trials;
    plot(x, cum_BER(i, :))
    legend(windows)
end
