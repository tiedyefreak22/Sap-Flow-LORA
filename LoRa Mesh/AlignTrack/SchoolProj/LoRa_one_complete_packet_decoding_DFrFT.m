...
%% START AlignTrack Trials
    ...
    for window = 1:1:length(windows)
        ...
        while (shift(i) + chirp_len - 1) <= length(received_signal)
            ...
            a = 1; % value of 1 should yield same result as regular DFT
            [f, received_fft_abs, received_fft_imag] = LoRa_demod_frft(input_sig, fc, SF, BW, Fs / decimation_factor, cfo, string(windows(window)), a);
